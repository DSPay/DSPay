...
static void DebugPrintInit() {
	...
	if (logfiles.count("all-to-debug")) {
		logfiles.clear();
		logfiles.insert("all-to-debug");
	}
...
//		}
		...
		it = g_DebugLogs.find("all-to-debug");
		it = g_DebugLogs.find((NULL == category || it != g_DebugLogs.end()) ? ("debug") : (category));
